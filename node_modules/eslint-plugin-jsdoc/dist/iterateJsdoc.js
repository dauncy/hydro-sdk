...
const getUtils = (node, jsdoc, jsdocNode, {
  ...
  utils.getPreferredTagName = (name, allowObjectReturn = false, defaultMessage = `Unexpected tag \`@${name}\``) => {
    const ret = _jsdocUtils.default.getPreferredTagName(name, tagNamePreference);
  ...
  utils.avoidDocsParamConditionally = param => {
    ...
    return (param && overrideReplacesDocs || !param && overrideReplacesDocs !== false) && (utils.hasTag('override') || utils.classHasTag('override')) || (param && implementsReplacesDocs || !param && implementsReplacesDocs !== false) && (utils.hasTag('implements') || utils.classHasTag('implements'));
  };

  utils.avoidDocs = param => {
    if (param && utils.avoidDocsParamOnly() || utils.avoidDocsParamConditionally(param) || // inheritdoc implies that all documentation is inherited; see https://jsdoc.app/tags-inheritdoc.html
  ...
  utils.hasReturnValue = (ignoreAsync = false) => {
    return _jsdocUtils.default.hasReturnValue(node, context, ignoreAsync);
  };
...
const getSettings = context => {
  ...
  settings.ignorePrivate = Boolean(_lodash.default.get(context, 'settings.jsdoc.ignorePrivate')); // `check-tag-names` and many require/param rules

  settings.tagNamePreference = _lodash.default.get(context, 'settings.jsdoc.tagNamePreference') || {}; // `require-param`, `require-description`, `require-example`, `require-returns`
  ...
  settings.augmentsExtendsReplacesDocs = _lodash.default.get(context, 'settings.jsdoc.augmentsExtendsReplacesDocs'); // `check-tag-names` only

  settings.additionalTagNames = _lodash.default.get(context, 'settings.jsdoc.additionalTagNames') || {}; // `check-examples` only

  settings.exampleCodeRegex = _lodash.default.get(context, 'settings.jsdoc.exampleCodeRegex') || null;
  settings.rejectExampleCodeRegex = _lodash.default.get(context, 'settings.jsdoc.rejectExampleCodeRegex') || null;
  settings.matchingFileName = _lodash.default.get(context, 'settings.jsdoc.matchingFileName') || null;
  settings.baseConfig = _lodash.default.get(context, 'settings.jsdoc.baseConfig') || {};
  settings.configFile = _lodash.default.get(context, 'settings.jsdoc.configFile');
  settings.eslintrcForExamples = _lodash.default.get(context, 'settings.jsdoc.eslintrcForExamples') !== false;
  settings.allowInlineConfig = _lodash.default.get(context, 'settings.jsdoc.allowInlineConfig') !== false;
  settings.reportUnusedDisableDirectives = _lodash.default.get(context, 'settings.jsdoc.reportUnusedDisableDirectives') !== false;
  settings.captionRequired = Boolean(_lodash.default.get(context, 'settings.jsdoc.captionRequired'));
  settings.noDefaultExampleRules = Boolean(_lodash.default.get(context, 'settings.jsdoc.noDefaultExampleRules')); // `require-param` only (deprecated)

  settings.allowOverrideWithoutParam = _lodash.default.get(context, 'settings.jsdoc.allowOverrideWithoutParam');
  settings.allowImplementsWithoutParam = _lodash.default.get(context, 'settings.jsdoc.allowImplementsWithoutParam');
  settings.allowAugmentsExtendsWithoutParam = _lodash.default.get(context, 'settings.jsdoc.allowAugmentsExtendsWithoutParam'); // `valid-types` only

  settings.allowEmptyNamepaths = _lodash.default.get(context, 'settings.jsdoc.allowEmptyNamepaths') !== false;
  settings.checkSeesForNamepaths = Boolean(_lodash.default.get(context, 'settings.jsdoc.checkSeesForNamepaths')); // `require-returns` only

  settings.forceRequireReturn = Boolean(_lodash.default.get(context, 'settings.jsdoc.forceRequireReturn')); // `require-example` only

  settings.avoidExampleOnConstructors = Boolean(_lodash.default.get(context, 'settings.jsdoc.avoidExampleOnConstructors'));
  return settings;
};
/**
      ...
      return {
        'Program:exit'() {
          ...
          comments.forEach(comment => {
            ...
            const indent = _lodash.default.repeat(' ', comment.loc.start.column);
