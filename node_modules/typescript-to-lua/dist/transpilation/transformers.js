...
function loadTransformersFromOptions(program, diagnostics) {
    ...
    for (const [index, transformerImport] of options.plugins.entries()) {
        ...
        const { error: resolveError, result: factory } = utils_1.resolvePlugin("transformer", `${optionName}.transform`, utils_1.getConfigDirectory(options), transformerImport.transform, transformerImport.import);
        if (resolveError)
