...
function transformFunctionLikeDeclaration(node, context) {
    ...
    if (ts.isFunctionExpression(node) && node.name && scope.referencedSymbols) {
        ...
        if (symbol) {
            ...
            const isReferenced = [...scope.referencedSymbols].some(([, nodes]) => nodes.some(n => { var _a; return ((_a = context.checker.getSymbolAtLocation(n)) === null || _a === void 0 ? void 0 : _a.valueDeclaration) === symbol.valueDeclaration; }));
...
exports.transformFunctionDeclaration = (node, context) => {
    ...
    const [body, functionScope] = node.asteriskToken
        ? generator_1.transformGeneratorFunctionBody(context, node.parameters, node.body, restParamName)
        : transformFunctionBody(context, node.parameters, node.body, restParamName);
    const block = lua.createBlock(body);
    const functionExpression = lua.createFunctionExpression(block, params, dotsLiteral, lua.FunctionExpressionFlags.Declaration);
