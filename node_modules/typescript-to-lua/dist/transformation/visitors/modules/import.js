...
const getAbsoluteImportPath = (relativePath, directoryPath, options) => relativePath[0] !== "." && options.baseUrl
...
function shouldBeImported(context, importNode) {
    const annotations = annotations_1.getTypeAnnotations(context.checker.getTypeAtLocation(importNode));
    return (context.resolver.isReferencedAliasDeclaration(importNode) &&
        !annotations.has(annotations_1.AnnotationKind.Extension) &&
        !annotations.has(annotations_1.AnnotationKind.MetaExtension));
}
