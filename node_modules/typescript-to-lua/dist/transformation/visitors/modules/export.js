...
function transformExportAllFrom(context, node) {
    ...
    const body = lua.createBlock([
        lua.createAssignmentStatement(lua.createTableIndexExpression(lua_ast_1.createExportsIdentifier(), forKey), forValue),
    ]);
    const pairsIdentifier = lua.createIdentifier("pairs");
    const forIn = lua.createForInStatement(body, [lua.cloneIdentifier(forKey), lua.cloneIdentifier(forValue)], [lua.createCallExpression(pairsIdentifier, [lua.cloneIdentifier(tempModuleIdentifier)])]);
...
function transformExportSpecifiersFrom(context, statement, moduleSpecifier, exportSpecifiers) {
    ...
    const [block] = block_1.transformScopeBlock(context, ts.createBlock([importDeclaration]), scope_1.ScopeType.Block);
