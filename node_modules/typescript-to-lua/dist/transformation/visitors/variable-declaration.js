        ...
        if (element.dotDotDotToken) {
            ...
            if (isObjectBindingPattern) {
                ...
                const usedProperties = elements.map(e => {
                    var _a, _b;
                    return lua.createTableFieldExpression(lua.createBooleanLiteral(true), lua.createStringLiteral(((_a = e.propertyName) !== null && _a !== void 0 ? _a : e.name).text, (_b = e.propertyName) !== null && _b !== void 0 ? _b : e.name));
                });
                expression = lualib_1.transformLuaLibFunction(context, lualib_1.LuaLibFeature.ObjectRest, undefined, tableExpression, lua.createTableExpression(usedProperties));
            }
            else {
            ...
            expression = lua.createTableIndexExpression(tableExpression, isObjectBindingPattern ? propertyName : lua.createNumericLiteral(index + 1));
        }
...
function transformBindingVariableDeclaration(context, bindingPattern, initializer) {
    ...
    if (ts.isObjectBindingPattern(bindingPattern) || bindingPattern.elements.some(isComplexBindingElement)) {
        ...
        if (initializer !== undefined && ts.isIdentifier(initializer)) {
            ...
            if (initializer) {
                statements.push(lua.createVariableDeclarationStatement(table, context.transformExpression(initializer)));
            }
        }
